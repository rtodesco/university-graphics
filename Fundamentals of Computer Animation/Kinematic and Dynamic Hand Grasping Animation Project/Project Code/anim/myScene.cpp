...
void MakeScene(void)
{
	...
	HandSystem* handSys = new HandSystem("handSys");
	success = GlobalResourceManager::use()->addSystem(handSys, true);
	assert(success);

	HandSimulator* handSim = new HandSimulator("handSim",handSys);
	success = GlobalResourceManager::use()->addSimulator(handSim,true);
	assert(success);
